...
4. 如无特殊情况，数组、对象、函数、不要用 `new`，直接使用字面量来初始化；
```
  let a = [];               √
  let o = {};               √ 
  function f() {};          √
...
const fs = require("fs");
```
9. 尽量少用异步操作，用 `async` / `await` 的同步方式来替代；
10. 一个变量尽可能只负责一种数据类型，不要随意变更，如：
```
let abc = "A,B,C";
abc = abc.split(",");           X
